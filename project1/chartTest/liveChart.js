 ...
 */
 ...
 for (i = 0; i <= 50; i++) {
     visits -= Math.round((Math.random() < 0.5 ? 1 : -1) * Math.random() * 10); //-10 ~ 10
 ...
 dateAxis.renderer.minGridDistance = 30; //y축 그리드 촘촘한 정도 조절(낮을수록 촘촘해짐)
 dateAxis.dateFormats.setKey("second", "ss");
 //초,분,시 가 바뀔 때 dateFormat.setkey 조건이 안먹어서 구간이 바뀔때 새로 지정
 dateAxis.periodChangeDateFormats.setKey("second", "[bold]h:mm a");
 dateAxis.periodChangeDateFormats.setKey("minute", "[bold]h:mm a");
 dateAxis.periodChangeDateFormats.setKey("hour", "[bold]h:mm a"); 
 dateAxis.renderer.inside = true; //X축 date 출력을 차트 안에 표시
 dateAxis.renderer.axisFills.template.disabled = true; //격자선 사이의 색을 지움
